...
<div class="row">
	 ...
	 @foreach ($data['item'] as $key => $item)
	 	...
	 	<a class="btn btn-primary" href="{{route('address.edit',['id' => $item->id])}}"> edit</a>
	 	<a class="btn btn-primary" href="{{route('address.detail',['id' => $item->id])}}"> detail</a>
	 	<a class="btn btn-primary" href="#" onclick="document.getElementById('delete-product-recently-{{$item->id}}').submit();"> delete</a>
	 	{{Form::open(['route' => 'address.post.delete','style' =>'display:none;','id' => 'address-'.$item->id])}}
